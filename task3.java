...
public class task3 {
   public static void heapSort(int[] myArray, int length) {
      int temp;
      int size = length-1;
      for (int i = (length / 2); i >= 0; i--)  heapify(myArray, i, size);
   ...
   public static void heapify (int [] myArray, int i, int heapSize) {
      ...
      int b = 2*i+1;
      int largestElement;
      if (a<= heapSize&&myArray[a] > myArray[i]) largestElement = a;
      else largestElement = i;      
      if (b <= heapSize&&myArray[b] > myArray[largestElement]) largestElement = b;
      if (largestElement != i) {
         int temp = myArray[i];
